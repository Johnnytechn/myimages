...
Azure Stream Analytics loads reference data in memory to achieve low-latency stream processing. To make use of reference data in your Stream Analytics job, you'll generally use a [reference data join](https://learn.microsoft.com/stream-analytics-query/reference-data-join-azure-stream-analytics) in your query.
...
To configure your SQL Database reference data, you first need to create reference data input. The following table explains each property you need to provide when you create the reference data input with its description.
...
|Database|The SQL Database instance that contains your reference data. For SQL Managed Instance, you must specify the port 3342. An example is *sampleserver.public.database.chinacloudapi.cn,3342*.|
...
Support for compression isn't available for reference data.

...
The benefit of not running these jobs continuously will be **cost savings**, as Stream Analytics jobs are [billed](https://www.azure.cn/pricing/details/stream-analytics/) per Streaming Unit **over time.**
...
For *ASA Resource Management*, we can use either the [REST API](https://learn.microsoft.com/rest/api/streamanalytics/), the [.NET SDK](https://learn.microsoft.com/dotnet/api/microsoft.azure.management.streamanalytics) or one of the CLI libraries ([Az CLI](https://learn.microsoft.com/cli/azure/ext/stream-analytics/stream-analytics), [PowerShell](https://learn.microsoft.com/powershell/module/az.streamanalytics)).
...
For this article, we decided to implement auto-pause in **PowerShell**. The first reason is that [PowerShell](https://learn.microsoft.com/powershell/scripting/overview) is now cross-platform. It can run on any OS, which makes deployments easier. The second reason is that it takes and returns objects rather than strings. Objects make parsing and processing easy for automation tasks.

In PowerShell, we'll use the [Az PowerShell](https://learn.microsoft.com/powershell/azure/new-azureps-module-az) module, which embarks [Az.Monitor](https://learn.microsoft.com/powershell/module/az.monitor/) and [Az.StreamAnalytics](https://learn.microsoft.com/powershell/module/az.streamanalytics/), for everything we need:

- [Get-AzStreamAnalyticsJob](https://learn.microsoft.com/powershell/module/az.streamanalytics/get-azstreamanalyticsjob) for the current job status
- [Start-AzStreamAnalyticsJob](https://learn.microsoft.com/powershell/module/az.streamanalytics/start-azstreamanalyticsjob) / [Stop-AzStreamAnalyticsJob](https://learn.microsoft.com/powershell/module/az.streamanalytics/stop-azstreamanalyticsjob)
- [Get-AzMetric](https://learn.microsoft.com/powershell/module/az.monitor/get-azmetric) with `InputEventsSourcesBacklogged` [(from ASA metrics)](../azure-monitor/essentials/metrics-supported.md#microsoftstreamanalyticsstreamingjobs)
- [Get-AzActivityLog](https://learn.microsoft.com/powershell/module/az.monitor/get-azactivitylog) for event names beginning with `Stop Job`
...
For the sake of brevity, here we'll illustrate the process in the [Azure portal](https://portal.azure.cn).

## Writing the PowerShell script locally

The best way to develop the script is locally. PowerShell being cross-platform, the script can be written and tested on any OS. On Windows we can use [Windows Terminal](https://www.microsoft.com/p/windows-terminal/9n0dx20hk701) with [PowerShell 7](https://learn.microsoft.com/powershell/scripting/install/installing-powershell-on-windows), and [Az PowerShell](https://learn.microsoft.com/powershell/azure/install-az-ps).
...
The full procedure is [here](../azure-functions/functions-create-function-app-portal.md#create-a-function-app), but the gist is to go in the [Azure portal](https://portal.azure.cn), and create a new Function App with:
...
- [Azure Stream Analytics Management .NET SDK](https://learn.microsoft.com/previous-versions/azure/dn889315(v=azure.100))
